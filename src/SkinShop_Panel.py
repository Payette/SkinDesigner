...
Refer to PanelLib API for Definition input functions
    Args:
        ...
        ShadingData: inputs one or more outputs from shading-type components: Shade, LouverShading, ShadingBox
...
ghenv.Component.Name = "SkinShop_Panel"
...
for input in range(numInputs):
    ...
    elif input == 8: inputName = 'ShadingData' ; access = accessList; typeHint = typeGHDoc
...
def PanelCreate():
    ...
    if numInputs > 8 and ShadingData: tmpShadeData = ShadingData
    if tmpShadeData : 
        tmpWindowData = None
